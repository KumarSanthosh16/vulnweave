...
/** A reviewable, portable baseline for thresholds and temporary risk acceptances. */
export interface VulnWeaveBaselinePolicy {
  schemaVersion: 1;
  failOn?: Severity;
...
export async function loadBaselinePolicy(rootDir: string, filename = "vulnweave.baseline.json"): Promise<VulnWeaveBaselinePolicy | undefined> {
  const path = join(rootDir, filename);
  try {
    return validateBaselinePolicy(JSON.parse(await readFile(path, "utf8")));
  } catch (error) {
    if (isMissingFile(error)) return undefined;
    throw new Error(`Invalid VulnWeave baseline policy at ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export function validateBaselinePolicy(value: unknown): VulnWeaveBaselinePolicy {
  if (!isRecord(value)) throw new Error("expected a JSON object");
  if (value.schemaVersion !== 1) throw new Error("schemaVersion must be 1");
  if (value.failOn !== undefined && !["critical", "high", "medium", "low", "info"].includes(stringValue(value.failOn))) throw new Error("failOn must be critical, high, medium, low, or info");
...
function isDate(value: unknown): boolean { return typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(new Date(`${value}T00:00:00.000Z`).getTime()); }
